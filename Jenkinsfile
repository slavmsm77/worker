...
pipeline{
    ...
    stages{
        ...
        stage("Build Docker image"){
            steps{
                script{
                    sh "docker build . -t ${registry}/${ms}:${tag} --platform \"linux/amd64/v8\" --build-arg=\"BUILDPLATFORM=linux/amd64/v8\" --build-arg=\"TARGETPLATFORM=linux/amd64/v8\" --build-arg=\"TARGETARCH=amd64\"  "
                }
            }
        }
